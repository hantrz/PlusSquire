...
const whatYouGet = [
  { icon: Paintbrush,  label: 'Fully custom design, not a theme store template' },
  { icon: Smartphone,  label: 'Built mobile-first and speed-optimized from day one' },
  { icon: ShoppingBag, label: 'Conversion-focused product, cart & checkout flow' },
  { icon: Code2,       label: 'Clean, documented code your team can maintain' },
]
    ...
    <>
      ...
      <main style={{ paddingTop: '68px' }}>
        <style>{`
          ...
          .std-whatget-head { text-align: center; max-width: 720px; margin: 0 auto 48px; }
          .std-whatget-tag {
            display: inline-flex; align-items: center; gap: 8px; background: var(--gl); color: var(--gd);
            font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: .06em;
            padding: 6px 14px; border-radius: 100px; margin-bottom: 16px;
          }
          .std-whatget-head h2 { font-size: clamp(20px, 2.4vw, 32px); margin-bottom: 0; white-space: nowrap; }
          @media(max-width:560px){
            .std-whatget-head h2 { white-space: normal; font-size: clamp(20px, 5.5vw, 24px); }
          }
          ...
          .std-whatget-item:not(:last-child)::after {
          ...
          .std-whatget-item p { font-size: 14.5px; font-weight: 600; color: var(--ink2); line-height: 1.5; max-width: 200px; }

          @media(max-width:820px){
            .std-whatget-row { flex-direction: column; align-items: center; gap: 32px; }
            .std-whatget-item { padding: 0; }
            .std-whatget-item:not(:last-child)::after { display: none; }
          }
        ...
        <section className="std-whatget">
          <div className="wrap">
            <div className="std-whatget-head" data-reveal="up">
              <div className="std-whatget-tag">What You Get</div>
              <h2>Everything included, <em style={{ color: 'var(--g)', fontStyle: 'normal' }}>nothing left to guess.</em></h2>
            </div>
            <div className="std-whatget-row">
              {whatYouGet.map((item, i) => (
                <div key={item.label} className="std-whatget-item" data-reveal="up" style={{ transitionDelay: `${i * 90}ms` }}>
                  <div className="std-whatget-ico"><item.icon size={22} strokeWidth={1.75} /></div>
                  <p>{item.label}</p>
                </div>
              ))}
            </div>
