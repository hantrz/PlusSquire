...
const whatYouGet = [
  { icon: Gauge,      label: 'Full Core Web Vitals & PageSpeed audit' },
  { icon: ImageIcon,  label: 'Image & asset optimization across the store' },
  { icon: Wrench,     label: 'App & script bloat identified & removed' },
  { icon: FileText,   label: 'Before/after performance report included' },
]
    ...
    <>
      ...
      <main style={{ paddingTop: '68px' }}>
        <style>{`
          ...
          .sso-whatget-head { text-align: center; max-width: 720px; margin: 0 auto 48px; }
          .sso-whatget-tag {
            display: inline-flex; align-items: center; gap: 8px; background: var(--gl); color: var(--gd);
            font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: .06em;
            padding: 6px 14px; border-radius: 100px; margin-bottom: 16px;
          }
          .sso-whatget-head h2 { font-size: clamp(20px, 2.4vw, 32px); margin-bottom: 0; white-space: nowrap; }
          @media(max-width:560px){
            .sso-whatget-head h2 { white-space: normal; font-size: clamp(20px, 5.5vw, 24px); }
          }
          ...
          .sso-whatget-item:not(:last-child)::after {
          ...
          .sso-whatget-item p { font-size: 14.5px; font-weight: 600; color: var(--ink2); line-height: 1.5; max-width: 200px; }

          @media(max-width:820px){
            .sso-whatget-row { flex-direction: column; align-items: center; gap: 32px; }
            .sso-whatget-item { padding: 0; }
            .sso-whatget-item:not(:last-child)::after { display: none; }
          }
        ...
        <section className="sso-whatget">
          <div className="wrap">
            <div className="sso-whatget-head" data-reveal="up">
              <div className="sso-whatget-tag">What You Get</div>
              <h2>Everything included, <em style={{ color: 'var(--g)', fontStyle: 'normal' }}>nothing left to guess.</em></h2>
            </div>
            <div className="sso-whatget-row">
              {whatYouGet.map((item, i) => (
                <div key={item.label} className="sso-whatget-item" data-reveal="up" style={{ transitionDelay: `${i * 90}ms` }}>
                  <div className="sso-whatget-ico"><item.icon size={22} strokeWidth={1.75} /></div>
                  <p>{item.label}</p>
                </div>
              ))}
            </div>
